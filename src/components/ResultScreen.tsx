...
export function ResultScreen({ result }: Props): ReactElement { return <main className="result-page"><img className="header-logo header-logo--result" src={imagePath(ImageName.HeaderLogo)} alt="Похудей ка" /><section className="result-content"><img className="result-card__image" src={imagePath(result.image)} alt={`Результат теста: ${result.title}`} /><p className="result-lead">{result.description}</p><p>{result.recommendation}</p><a className="result-link" href="https://leonov-chef.com/pohodeyka?utm_source=calen&utm_medium=prodaja&utm_campaign=pohodeyka&utm_content=link"><img src={imagePath(ImageName.LinkButton)} alt="Хочу худеть легко уже сейчас" /></a></section><footer className="footer"><img src={imagePath(ImageName.FooterInfo)} alt="Информация о программе Похудей ка" /></footer></main>; }
